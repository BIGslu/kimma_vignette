...
We see that each model is the best fit for a subset of genes. Batch improves the model fit for only about 18% of genes, and this improvement is moderate (*e.g.* teal dots farther from the 1:1 line and mean $\Delta$ AIC = 2.3). Also of importance, the model without batch does not have strong evidence of improved fit for the rest of the genes with a mean $\Delta$ AIC = 1.5.
...
kmFit(dat = example.voom, 
      ...
      run.lme = TRUE,
      use.weights = TRUE,
      subset.var = list("asthma"),
      subset.lvl = list("healthy"))
```

You can specify multiple subset variables and levels using lists where the first `subset.var` corresponds to the first `subset.lvl` vector. For example, this selects all of these data.
...
kmFit(dat = example.voom, 
      ...
      run.lme = TRUE,
      use.weights = TRUE,
      subset.var = list("virus", "asthma"),
      subset.lvl = list(c("none", "HRV"),
                        c("healthy", "asthma")))
```
